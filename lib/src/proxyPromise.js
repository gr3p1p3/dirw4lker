...
function proxyPromise(config) {
    let {target, targetHost, targetPort, pAddress, PORT: port, PAYLOAD, maxChunks, timeout, injectPayload, isTargetHttps} = config;
    //setting default params
    maxChunks = maxChunks || PAYLOAD.length;
    ...
    return new Promise(function (resolve, reject) {
        ...
        client.connect(port, pAddress, function onConnectionOpen() {
        ...
        client.on('data', function (data) {
            chunksCounter++;
            if(!isTargetHttps && chunksCounter > 1) { // dont need to append proxy response... this will be 200 at first
