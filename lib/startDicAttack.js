        ...
        for (let ext of extensions) {
            ...
            const requestOptions = {
                ...
                onResponse: function onResponse(erroredResult, foundResult) {
                    ...
                    if (!REQUESTS_STATS.min) {
                        REQUESTS_STATS.min = ms;
                    }
                    if (!REQUESTS_STATS.min) {
                        REQUESTS_STATS.min = ms;
                    }
                    if (!REQUESTS_STATS.median) {
                        REQUESTS_STATS.median = (REQUESTS_STATS.min + REQUESTS_STATS.max) / 2;
                    }
                    ...
                    logger.overwriteLastLine(percent.toFixed(0) + '%'
                        ...
                        + '\t' + 'ETA: ' + Math.ceil((REQUESTS_STATS.median * (listLen - REQUESTS_STATS.counter)) / 1000) + 's'
                        + '\t' + targetPath);
